#!/usr/bin/env zsh
#
# install-powerlevel10k.zsh — install the powerlevel10k theme and its prerequisites.
#
# Standalone: run it on its own, after install-oh-my-zsh.zsh (it clones
# the theme into ~/.oh-my-zsh/custom, which Oh My Zsh creates).
#
# Usage:
...
#   2. Configures Terminal.app, right after the fonts are in place, by
...
#
# Design notes:
#   * Requires Oh My Zsh first (./install-oh-my-zsh.zsh): the theme lives
#     in ${ZSH_CUSTOM:-$HOME/.oh-my-zsh/custom}/themes.
#   * Idempotent: safe to re-run; each step no-ops if already done, and a
#     re-run never rewrites ~/.zshrc unless the ZSH_THEME value actually
#     differs.
#   * git is required (from the Command Line Tools); Homebrew is required
#     (for the fonts); no sudo.
...
install_powerlevel10k() {
    ...
    # The theme lands in Oh My Zsh's custom dir, so OMZ must be there first.
    if [ ! -d "${HOME}/.oh-my-zsh" ]; then
        error "Oh My Zsh not found (expected ${HOME}/.oh-my-zsh) — run ./install-oh-my-zsh.zsh first, then re-run this script."
        return 1
    fi
...
main() {
    ...
    install_powerlevel10k
    set_zsh_theme
    say "powerlevel10k setup complete."
